...
login = robin_stocks.robinhood.login("", "")
...
def run(sc):
    ...
    r = robin_stocks.robinhood.crypto.get_crypto_quote(ticker, info="mark_price")
    ...
    if counter1 < 6:
        ...
        if counter1 == 6: 
            if float(SE3P[0])*1.005 > float(r) and float(r) < float(average_cost-float(average_cost*float(0.015))):
                ...
                Shares2Buy = math.floor(float(5) / float(r)-1)
    ...
    if counter2 < 6:
        ...
        if counter2 == 6:
            if float(SE3P[0])*1.0115 < float(r) and float(r) > float(average_cost+float(average_cost*float(0.04))):
                ...
                Shares2Sell = math.floor(float(10) / float(r)+1)
    ...
    # exits condition to prevent skewed buys such that number of shares reaches below 0
    if num_shares <= 0:
        sys.exit()
...
def crypto_BUY(ticker, amountD):
    r = robin_stocks.robinhood.orders.order_buy_crypto_by_quantity(ticker, amountD)
    print(r)

def crypto_SELL(ticker, amountD):
    r = robin_stocks.robinhood.orders.order_sell_crypto_by_quantity(ticker, amountD)
    print(r)
