...
from robin_stocks.crypto import get_crypto_quote
from matplotlib import pyplot

# Program description:
# A Robinhood bot created to automatically monitor and
# trade crypto currency currently supported by Robinhood.
#
# This bot runs a scheduler every 60 seconds in order to
# update the prices on a 60 second interval for a
# list that will hold the previous prices for past 6 minute.
#
# This bot [REQUIRES] a individual to already have SET amount of shares of the current
# crypto they want to trade.
#
...
# Some buying and selling errors will occur if a individual does not have enough shares
# to sell or enough money to buy. If errors occur, simply re-update through redoing
# instructions above and restart the program.
...
# this, along with the ac_ceiling and ac_floor will help ensure that the program will
# only sell for profit, and buy to average down
average_cost = 1.26

# average cost ceiling percentage (ensures there is a large enough disparity between the
# live price and the required minimal amount that is greater than the live price to
# obtain profitability when selling) average cost floor percentage(ensures there is a
# large enough disparity between the live price and the required minimal amount that is
# lower than the live price to obtain profitability when buying)
ac_ceiling = 0.0075
ac_floor = 0.02
y = []
yy = []
...
def run(sc):
    ...
    cryptoSymbol = get_crypto_quote(ticker, info="mark_price")
    ...
    pastPredictionMinutesAhead1,
    pastPredictionPrice1,
    pastPredictionDate1 = removeExpiredPredictions(
        pastPredictionMinutesAhead1,
        pastPredictionPrice1,
        pastPredictionDate1
    )
    ...
    xyz1flag30,
    xyz1flag60,
    xyz1flag120 = checkPastPredictions(
        pastPredictionMinutesAhead1,
        pastPredictionPrice1,
        pastPredictionDate1,
        cryptoSymbol,
    ...
    if numOfRuns < 7:
        ...
        if numOfRuns == 6:
            if (float(historicalPrices[0])*0.993 > float(cryptoSymbol)
                    and float(cryptoSymbol)
                    < float(average_cost-float(average_cost*float(ac_floor))
                            )
                    ):
                priceCompareBuyFlag = True

        elif numOfRuns == 5:
            if (float(historicalPrices[0])*0.994 > float(cryptoSymbol)
                    and float(cryptoSymbol)
                    < float(average_cost-float(average_cost*float(ac_floor)))
                    ):
                priceCompareBuyFlag = True
            ...
            if (float(historicalPrices[0])*0.995 > float(cryptoSymbol)
                    and float(cryptoSymbol)
                    < float(average_cost-float(average_cost*float(ac_floor)))
                    ):
                priceCompareBuyFlag = True
            ...
            if (float(historicalPrices[0])*0.996 > float(cryptoSymbol)
                and float(cryptoSymbol)
                < float(average_cost-float(average_cost*float(ac_floor)))
                ):
                priceCompareBuyFlag = True
            ...
            if (float(historicalPrices[0])*0.997 > float(cryptoSymbol)
                        and float(cryptoSymbol)
                        < float(average_cost-float(average_cost*float(ac_floor)))
                    ):
                priceCompareBuyFlag = True
    ...
    if numOfRuns >= 7:
        ...
        if (float(cryptoSymbol)*1.07 < float(historicalPrices[0])
                    and float(cryptoSymbol)
                    < float(average_cost-float(average_cost*float(ac_floor)))
                ):
            priceCompareBuyFlag = True

        elif (float(cryptoSymbol)*1.006 < float(historicalPrices[1])
                and float(cryptoSymbol)
                < float(average_cost-float(average_cost*float(ac_floor)))
              ):
            priceCompareBuyFlag = True

        elif (float(cryptoSymbol)*1.005 < float(historicalPrices[2])
                and float(cryptoSymbol)
                < float(average_cost-float(average_cost*float(ac_floor)))
              ):
            priceCompareBuyFlag = True

        elif (float(cryptoSymbol)*1.004 < float(historicalPrices[3])
                and float(cryptoSymbol)
                < float(average_cost-float(average_cost*float(ac_floor)))
              ):
            priceCompareBuyFlag = True

        elif (float(cryptoSymbol)*1.003 < float(historicalPrices[4])
                and float(cryptoSymbol)
                < float(average_cost-float(average_cost*float(ac_floor)))
              ):
            priceCompareBuyFlag = True

        elif (float(cryptoSymbol)*1.002 < float(historicalPrices[5])
                and float(cryptoSymbol)
                < float(average_cost-float(average_cost*float(ac_floor)))
              ):
            priceCompareBuyFlag = True
    ...
    if numOfRuns < 7:
        ...
        if numOfRuns == 6:
            if (float(historicalPrices[0])*1.007 < float(cryptoSymbol)
                and float(cryptoSymbol)
                > float(average_cost+float(average_cost*float(ac_ceiling)))
                ):
                priceCompareSellFlag = True

        elif numOfRuns == 5:
            if (float(historicalPrices[0])*1.006 < float(cryptoSymbol)
                and float(cryptoSymbol)
                > float(average_cost+float(average_cost*float(ac_ceiling)))
                ):
                priceCompareSellFlag = True
            ...
            if (float(historicalPrices[0])*1.005 < float(cryptoSymbol)
                        and float(cryptoSymbol)
                        > float(average_cost+float(average_cost*float(ac_ceiling)))
                    ):
                priceCompareSellFlag = True
            ...
            if (float(historicalPrices[0])*1.004 < float(cryptoSymbol)
                    and float(cryptoSymbol)
                    > float(average_cost+float(average_cost*float(ac_ceiling)))
                ):
                priceCompareSellFlag = True
            ...
            if (float(historicalPrices[0])*1.003 < float(cryptoSymbol)
                    and float(cryptoSymbol)
                    > float(average_cost+float(average_cost*float(ac_ceiling)))
                    ):
                priceCompareSellFlag = True
    ...
    if numOfRuns >= 7:
        ...
        if (float(cryptoSymbol)*0.993 > float(historicalPrices[0])
                and float(cryptoSymbol)
                > float(average_cost+float(average_cost*float(ac_ceiling)))
                ):
            priceCompareSellFlag = True

        elif (float(cryptoSymbol)*0.994 > float(historicalPrices[1])
                and float(cryptoSymbol)
                > float(average_cost+float(average_cost*float(ac_ceiling)))
              ):
            priceCompareSellFlag = True

        elif (float(cryptoSymbol)*0.995 > float(historicalPrices[2])
              and float(cryptoSymbol)
              > float(average_cost+float(average_cost*float(ac_ceiling)))
              ):
            priceCompareSellFlag = True

        elif (float(cryptoSymbol)*0.996 > float(historicalPrices[3])
              and float(cryptoSymbol)
                > float(average_cost+float(average_cost*float(ac_ceiling)))
              ):
            priceCompareSellFlag = True

        elif (float(cryptoSymbol)*0.997 > float(historicalPrices[4])
              and float(cryptoSymbol)
              > float(average_cost+float(average_cost*float(ac_ceiling)))
              ):
            priceCompareSellFlag = True

        elif (float(cryptoSymbol)*0.998 > float(historicalPrices[5])
              and float(cryptoSymbol)
              > float(average_cost+float(average_cost*float(ac_ceiling)))
              ):
            priceCompareSellFlag = True
    ...
    if priceCompareSellFlag is True \
            and xyz1flag1 is True \
            and xyz1flag2 is True \
            and xyz1flag5 is True:
        Shares2Sell = math.floor(
            (float(shares2sellDollar)) / (float(cryptoSymbol)))
        order = robin_stocks.orders.order_sell_crypto_by_price(
            ticker, float(shares2sellDollar))
        print(order)
        print("\nsold: ", float(cryptoSymbol)*0.996)
        print("avg cost:", average_cost, "\n")
        updateSharesSell = float(shares2sellDollar) / \
            float(float(cryptoSymbol)*0.996)
        num_shares -= updateSharesSell

    elif priceCompareSellFlag is True \
            and xyz1flag15 is True \
            and xyz1flag30 is True \
            and xyz1flag60 is True \
            and xyz1flag120 is True:
        Shares2Sell = math.floor(
            (float(shares2sellDollar)) / (float(cryptoSymbol)))
        order = robin_stocks.orders.order_sell_crypto_by_price(
            ticker, float(shares2sellDollar))
    ...
    if priceCompareBuyFlag is True:
        Shares2Buy = math.floor(
            (float(shares2buyDollar)) / (float(cryptoSymbol)))
        order = robin_stocks.orders.order_buy_crypto_by_price(
            ticker, float(shares2buyDollar))
        print(order)
        ...
        updateSharesBuy = float(shares2buyDollar) / \
            float(float(cryptoSymbol)*1.0039)
...
def crypto_BUY(ticker, amountD):
    cryptoSymbol = robin_stocks.orders.order_buy_crypto_by_quantity(
        ticker, float(amountD), timeInForce='gtc')
    print(cryptoSymbol)
...
def crypto_SELL(ticker, amountD):
    cryptoSymbol = robin_stocks.orders.order_sell_crypto_by_quantity(
        ticker, float(amountD), timeInForce='gtc')
    print(cryptoSymbol)
    print("ticker.state: ", cryptoSymbol.state)
    return cryptoSymbol
    ...
    if len(pd) > 0:
        ...
        for i in pd:
            ...
            if checkRes == '-':
                ...
                checkResflag1 = True
                checkResflag2 = True
            if checkResflag1 is True:
                del pp[count]
                checkResflag1 = False
            if checkResflag2 is True:
                del pm[count]
                checkResflag2 = False
    return (pm, pp, pd)
...
def linear_Regression(xx, yy, pm, pp, pd):
    ...
    pyplot.savefig("tempmain_linear_regression.png")
...
def price_prediction(m, p, d):
    ...
    print("\nprice prediction", ((pts-ts)/60), "minutes ahead from now:$",
          price, " [ ", predictionMinutesAhead, " ] ")
    ...
    print("price prediction", ((pts2-ts)/60), "minutes ahead from now:$",
          price2, " [ ", predictionMinutesAhead2, " ] ")
    ...
    print("price prediction", ((pts3-ts)/60), "minutes ahead from now:$",
          price3, " [ ", predictionMinutesAhead3, " ] ")
    ...
    print("price prediction", ((pts4-ts)/60), "minutes ahead from now:$",
          price4, " [ ", predictionMinutesAhead4, " ] ")
    ...
    print("price prediction", ((pts5-ts)/60), "minutes ahead from now:$",
          price5, " [ ", predictionMinutesAhead5, " ] ")
    ...
    print("price prediction", ((pts6-ts)/60), "minutes ahead from now:$",
          price6, " [ ", predictionMinutesAhead6, " ] ")
    ...
    print("price prediction", ((pts7-ts)/60), "minutes ahead from now:$",
          price7, " [ ", predictionMinutesAhead7, " ] ", "\n")
