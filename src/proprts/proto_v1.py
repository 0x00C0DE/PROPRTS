...
def run(sc):
    ...
    cryptoSymbol = robin_stocks.crypto.get_crypto_quote(
        ticker, info="mark_price")
    ...
    print(
        "\nprice prediction",
        ((pts-ts)/60),
        "minutes ahead from now:$", price, " [ ", predictionMinutesAhead, " ] "
    )
    ...
    print(
        "price prediction",
        ((pts2-ts)/60), "minutes ahead from now:$", price2,
        " [ ", predictionMinutesAhead2, " ] "
    )
    ...
    pyplot.title("PROPRTS-1")
    pyplot.savefig("LR1-proto-V1.png")
